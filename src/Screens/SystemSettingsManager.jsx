...
import { getSystemSettings, updateSystemSetting } from '../utils/dummyData';
...
const SystemSettingsManager = () => {
  ...
  const fetchSettings = async () => {
    try {
      setLoading(true);
      const data = await getSystemSettings(authToken);
  ...
  const handleSave = async (settingKey) => {
    try {
      ...
      await updateSystemSetting(authToken, settingKey, editValue);
      await fetchSettings();
      setEditingKey(null);
      setEditValue('');
      ...
      setMessage({ type: 'error', text: error.message || 'Error updating setting' });
      setTimeout(() => setMessage({ type: '', text: '' }), 3000);
    } finally {
      setSaving(false);
    }
  ...
  const handleBooleanToggle = async (setting) => {
    ...
    try {
      ...
      await updateSystemSetting(authToken, setting.setting_key, newValue.toString());
      await fetchSettings();
      setMessage({ type: 'success', text: 'Setting updated successfully!' });
      setTimeout(() => setMessage({ type: '', text: '' }), 3000);
    } catch (error) {
